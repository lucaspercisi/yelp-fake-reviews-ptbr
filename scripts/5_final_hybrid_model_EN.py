# -*- coding: utf-8 -*-
"""
Created on Sat Nov 25 11:17:38 2023
...
def extract_words_from_tagged_content(tagged_content):
    try:
        ...
        processed_content = [f'{word.lower()}_{tag}' for word, tag in content_list if word.lower() not in string.punctuation and word.lower() not in stop_words_en and word.strip() != '']
        return ' '.join(processed_content)
    except ValueError:
        return ""
    
url_dataset_pt = 'https://raw.githubusercontent.com/lucaspercisi/yelp-fake-reviews-ptbr/main/Datasets/portuguese/yelp-fake-reviews-dataset-pt-pos-tagged.csv'
url_dataset_en = 'https://raw.githubusercontent.com/lucaspercisi/yelp-fake-reviews-ptbr/main/Datasets/english/yelp-fake-reviews-dataset-en.csv'
yelp_df = pd.read_csv(url_dataset_pt)
yelp_df_en = pd.read_csv(url_dataset_en)

# total_portugues = len(yelp_df)
# num_verdadeiras_portugues = len(yelp_df[yelp_df['fake_review'] == False])
# num_falsas_portugues = len(yelp_df[yelp_df['fake_review'] == True])


# Calculando o número de amostras para cada classe no dataset em inglês
num_verdadeiras_ingles = 14567
num_falsas_ingles = 14567

# Amostrando o dataset em inglês
df_ingles_verdadeiras = yelp_df_en[yelp_df_en['fake_review'] == False].sample(num_verdadeiras_ingles)
df_ingles_falsas = yelp_df_en[yelp_df_en['fake_review'] == True].sample(num_falsas_ingles)

# Combinando as amostras
df_ingles_ajustado = pd.concat([df_ingles_verdadeiras, df_ingles_falsas])
print(df_ingles_ajustado.shape)
print(df_ingles_ajustado[df_ingles_ajustado['fake_review'] == True].shape)
print(df_ingles_ajustado[df_ingles_ajustado['fake_review'] == False].shape)
...
df_ingles_ajustado['word_count'] = df_ingles_ajustado['content'].apply(lambda x: len(str(x).split(" ")))

#limpando conteudo textual
df_ingles_ajustado['cleaned_content'] = df_ingles_ajustado['content'].apply(clean_text)
...
yelp_df_sample = df_ingles_ajustado.copy()
...
def run_and_save_results(clf, X, y, classifier_name, vectorizer, results_df, features_useds):
    ...
    nome_arquivo = f"{caminho_base}/resultados_{vectorizer}_{classifier_name}_{agora}_en.csv"
